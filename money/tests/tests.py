...
from money.tests.models import Entity, Entity_0_USD, Entity_USD
...
class MoneyFieldTestCase(TestCase):
    ...
    def testRetreiveAndUpdate(self):
        created = Entity.objects.create(name="one hundred dollars", price=Money(100, "USD"))
        created.save()
        self.assertEquals(created.price, Money(100, "USD"))

        ent = Entity.objects.filter(price__exact=Money(100, "USD")).get()
        self.assertEquals(ent.price, Money(100, "USD"))
        ent.price.amount = 300
        ent.save()

        ent = Entity.objects.filter(price__exact=Money(300, "USD")).get()
    ...
    def testEdgeCases(self):
        created = Entity.objects.create(name="zero dollars", price=Money(0, "USD"))
        created.save()
        self.assertEquals(created.price, Money(0, "USD"))

        ent = Entity.objects.filter(price__exact=Money(0, "USD")).get()
        self.assertEquals(ent.price, Money(0, "USD"))
